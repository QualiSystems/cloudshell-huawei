...
CONFIGURE_V3_GROUP = CommandTemplate(
    "snmp-agent group v3 {group_name} {auth_type} {snmp_access}-view {view_name}"
)
CONFIGURE_V3_USER = CommandTemplate(
    "snmp-agent usm-user v3 {user_name} group {group_name}"
)
CONFIGURE_V3_AUTH = CommandTemplate(
    "snmp-agent usm-user v3 {user_name} authentication-mode {auth_protocol}"
)  # md5 | sha
CONFIGURE_V3_PRIV = CommandTemplate(
    "snmp-agent usm-user v3 {user_name} privacy-mode {priv_protocol}"
