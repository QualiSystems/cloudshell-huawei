...
class TestHuaweiRemoveVlanFlow(unittest.TestCase):
    def setUp(self):
        ...
        self.tested_instance = HuaweiRemoveVlanFlow(cli_handler=cli_handler, logger=logger)

    def tearDown(self):
        super(TestHuaweiRemoveVlanFlow, self).tearDown()
        del self.tested_instance

    @mock.patch("cloudshell.networking.huawei.flows.huawei_remove_vlan_flow.AddRemoveVlanActions")
    @mock.patch("cloudshell.networking.huawei.flows.huawei_remove_vlan_flow.SystemActions")
    def test_execute_flow_success(self, sys_actions_class, vlan_actions_class):
        ...
        sys_actions.display_running_config.side_effect = ["Device configuration before Removing VLAN Commands",
        ...
        sys_actions.clean_current_configuration_on_interface.assert_called_once_with(configuration="Device configuration before Removing VLAN Commands")
    ...
    def test_execute_flow_failed(self, sys_actions_class, vlan_actions_class):
        ...
        with self.assertRaisesRegexp(Exception, re.escape("[FAIL] VLAN(s) {} removing failed".format(vlan))):
            self.tested_instance.execute_flow(vlan, port_mode, full_port_name, qnq, ctag)
