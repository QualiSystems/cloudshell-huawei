...
class TestHuaweiAddVlanFlow(unittest.TestCase):
    ...
    @mock.patch("cloudshell.networking.huawei.flows.huawei_add_vlan_flow.AddRemoveVlanActions")
    ...
    def test_execute_flow_success_single(self, sys_actions_class, vlan_actions_class):
        ...
        sys_actions.display_running_config.side_effect = ["Device configuration before Add VLAN Commands",
    ...
    def test_execute_flow_success_range(self, sys_actions_class, vlan_actions_class):
        ...
        vlan_actions.set_vlan_range_to_interface.assert_called_once_with(start_vlan, end_vlan, port_mode)
    ...
    def test_execute_flow_fail(self, sys_actions_class, vlan_actions_class):
        ...
        with self.assertRaisesRegexp(Exception, re.escape("[FAIL] VLAN(s) {} configuration failed".format(vlan))):
            self.tested_instance.execute_flow(vlan, port_mode, full_port_name, qnq, ctag)
